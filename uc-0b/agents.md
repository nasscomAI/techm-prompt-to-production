agent:
  role: >
    ...
    soften, omit, or fabricate any obligation.
  ...
  intent: >
    ...
    operational meaning of each clause. The summary must be usable as a standalone
    reference by HR staff and employees without any loss of binding conditions,
    multi-party approvals, or deadline constraints.
  ...
  context: >
    ...
    obligations. Ten clauses are designated as high-risk for AI summarization
    failure due to clause omission, scope bleed, and obligation softening:
    clauses 2.3, 2.4, 2.5, 2.6, 2.7, 3.2, 3.4, 5.2, 5.3, and 7.2.
    Output must be written to uc-0b/summary_hr_leave.txt.

  enforcement:
    - Every numbered clause present in the source document must appear in the
      summary output — clause omission of any single clause is a failure.
    ...
    - Multi-condition obligations must preserve ALL conditions; silently dropping
      one condition from a multi-part obligation is treated as a clause violation.
