...
- [Y] `agents.md` committed for all 4 UCs
- [Y] `skills.md` committed for all 4 UCs
- [Y] `classifier.py` runs on `test_[city].csv` without crash
- [Y] `results_[city].csv` present in `uc-0a/`
- [Y] `app.py` for UC-0B, UC-0C, UC-X — all run without crash
- [Y] `summary_hr_leave.txt` present in `uc-0b/`
- [Y] `growth_output.csv` present in `uc-0c/`
- [Y] 4+ commits with meaningful messages following the formula
- [Y] All sections below are filled in
...
> [The highest-risk clauses a naive prompt would drop or weaken — §2.4 — drops "Verbal approval is not valid" (softening)
...
> Yes — ["as is standard practice"
"typically in government organisations"
"employees are generally expected to"]

**Your git commit message for UC-0B:**

> [UC-0B Fix clause omission + obligation softening: naive prompt dropped conditions 
and weakened binding verbs → added retrieve_policy + summarize_policy skills, 
enforcement rules in agents.md, verbatim flagging for high-risk clauses, 
ground-truth coverage check for all 10 clauses]
